...
public class CardanGrilleCipher {
    ...
    private void MakeChipherText (){
        ...
        for (int i = 0; i < size; i++){
            for (int j = 0; (j < size) && (index < OrigText.length()); j++){
                if ((Key[i][j]) && (MatrixForCoding[i][j] == 0)){
                    MatrixForCoding[i][j] = OrigText.charAt(index);
                    index++;
                }
            }
        }
        ...
        for (int i = 0; i < size; i++){
            jmatr = 0;
            for (int j = size-1; (j >=0) && (index < OrigText.length()); j--){
                if ((Key[j][i]) && (MatrixForCoding[imatr][jmatr] == 0)){
                    MatrixForCoding[imatr][jmatr] = OrigText.charAt(index);
                    index++;
                }
                jmatr++;
            }
