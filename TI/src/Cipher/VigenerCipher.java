...
public class VigenerCipher {
    ...
    public void setOriginalText (String sOriginalText){
        OrigText = sOriginalText.toUpperCase(Locale.ROOT);
    }
  ...
  //      showTableRu();
        ...
        int i;
        int j;
        while (index < OrigText.length()){
            if ( (OrigText.charAt(index) >= 65) && (OrigText.charAt(index) <= 90) ){
                j = OrigText.charAt(index) - 65;
                i = StraightKey.charAt(index) - 65;
                s.append(TableEn[i][j]);
                index++;
            }
            else {
                if ( ( OrigText.charAt(index) >= 1040) && (OrigText.charAt(index) <= 1072) ){
                    j = (OrigText.charAt(index) - 1040);
    ...
    private void MakeOriginalText() {
        ...
        OrigText = s.toString().toUpperCase(Locale.ROOT);
    }
